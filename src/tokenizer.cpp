...
bool matchesAt(std::string_view input, size_t position, std::string_view sequence)
{
  return position <= input.size() && sequence.size() <= input.size() - position &&
...
bool standaloneLineEnd(std::string_view input, size_t tagEnd, size_t * lineEnd)
{
  ...
  if (position < input.size()) {
    if (input[position] == '\n') {
      ++position;
    } else if (input[position] == '\r' && position + 1 < input.size() && input[position + 1] == '\n') {
      position += 2;
    } else {
      return false;
    }
...
{
  return type == Node::TypeSection || type == Node::TypeNegate || type == Node::TypeStop || type == Node::TypeComment ||
...
Node * appendNode(Node * parent, std::unique_ptr<Node> node, std::size_t * nodeCount, const Tokenizer::Limits& limits,
    int lineNo, int charNo)
{
  if (*nodeCount >= limits.maxNodes) {
    throw TokenizerException("Template node count limit exceeded", lineNo, charNo);
  }
...
void appendSourceOnlyOutput(Node * parent, std::string_view output, int flags, bool preserveEmpty,
    std::size_t * nodeCount, const Tokenizer::Limits& limits, int lineNo, int charNo)
{
  if (output.empty() && !preserveEmpty) {
    return;
  }
  appendNode(
      parent, std::make_unique<Node>(Node::TypeOutput, std::string(output), flags), nodeCount, limits, lineNo, charNo);
}
...
void Tokenizer::setStartSequence(const char * start, int len)
{
  ...
  setStartSequence(len == 0 ? std::string_view(start) : std::string_view(start, static_cast<size_t>(len)));
}
...
void Tokenizer::setStopSequence(const char * stop, int len)
{
  ...
  setStopSequence(len == 0 ? std::string_view(stop) : std::string_view(stop, static_cast<size_t>(len)));
}
...
void Tokenizer::tokenize(std::string * tmpl, Node * root, const Limits& limits, bool escapeOutput)
{
  if (tmpl == NULL) {
    throw Exception("Missing template");
  }
...
void Tokenizer::tokenize(std::string_view tmpl, Node * root, bool escapeOutput)
{
  tokenize(tmpl, root, Limits(), escapeOutput);
}

void Tokenizer::tokenize(std::string_view tmpl, Node * root, const Limits& limits, bool escapeOutput)
{
  if (root == NULL) {
  ...
  if (_startSequence.size() > limits.maxDelimiterBytes || _stopSequence.size() > limits.maxDelimiterBytes) {
  ...
  for (pos = 0; pos < tmplL; pos++) {
    ...
    if (!inTag) {
      ...
      if (tmpl[pos] == stopC && matchesAt(tmpl, pos, stop)) {
          ...
          case '=':
            if (buffer.at(buffer.length() - 1) != '=') {
              ...
              oss << "Missing closing delimiter (=)"
                  << " in tag starting at " << startLineNo << ":" << startCharNo;
              throw TokenizerException(oss.str(), startLineNo, startCharNo);
            }
            trim(buffer, " \f\n\r\t\v=");
            ...
            if (delims[0].size() > limits.maxDelimiterBytes || delims[1].size() > limits.maxDelimiterBytes) {
              throw TokenizerException("Template delimiter size limit exceeded", startLineNo, startCharNo);
            }
        ...
        const bool standaloneCandidate = !inTripleTag && (skip || isStandaloneType(currentType));
        if (standaloneCandidate && tagPrefixIsIndent && standaloneLineEnd(tmpl, pos + tmpStopL, &standaloneEnd)) {
          if (tagIndentationBytes > outputBeforeTag.size()) {
            throw TokenizerException("Invalid standalone parser state", startLineNo, startCharNo);
          }
          outputBeforeTag.resize(outputBeforeTag.size() - tagIndentationBytes);
        }

        if (!outputBeforeTag.empty()) {
          appendNode(nodeStack.back().node, std::make_unique<Node>(Node::TypeOutput, outputBeforeTag), &nodeCount,
              limits, startLineNo, startCharNo);
        }
        outputBeforeTag.clear();

        if (standaloneEnd != std::string_view::npos) {
          const int sourceFlags =
              Node::FlagLambdaOnly | (currentType == Node::TypePartial ? Node::FlagPartialIndent : Node::FlagNone);
          ...
          appendSourceOnlyOutput(nodeStack.back().node,
              tmpl.substr(tagStart - tagIndentationBytes, tagIndentationBytes), sourceFlags, true, &nodeCount, limits,
        ...
        if (!skip) {
          ...
          if (currentType & Node::TypeHasChildren) {
            ...
            if (currentDepth >= limits.maxNestingDepth) {
              throw TokenizerException("Template nesting limit exceeded", startLineNo, startCharNo);
            }
          } else if (currentType == Node::TypeStop) {
            if (nodeStack.size() <= 1) {
              std::ostringstream oss;
              oss << "Extra closing section '" << buffer << "' at " << startLineNo << ":" << startCharNo;
              throw TokenizerException(oss.str(), startLineNo, startCharNo);
            }

            const ParseFrame& openSection = nodeStack.back();
            if (!openSection.node->data.has_value() || *openSection.node->data != buffer) {
              std::ostringstream oss;
              oss << "Mismatched closing section '" << buffer << "' at " << startLineNo << ":" << startCharNo
                  << "; expected '" << (!openSection.node->data.has_value() ? std::string() : *openSection.node->data)
                  << "' opened at " << openSection.lineNo << ":" << openSection.charNo;
              throw TokenizerException(oss.str(), startLineNo, startCharNo);
            }
          }

          std::unique_ptr<Node> pendingNode = std::make_unique<Node>(currentType, buffer, currentFlags);
          ...
          node =
              appendNode(nodeStack.back().node, std::move(pendingNode), &nodeCount, limits, startLineNo, startCharNo);
          // Push/pop stack
          if (currentType & Node::TypeHasChildren) {
            nodeStack.push_back(ParseFrame(node, startLineNo, startCharNo));
          } else if (currentType == Node::TypeStop) {
            nodeStack.pop_back();
          }
        }
        ...
        if (standaloneEnd != std::string_view::npos) {
          appendSourceOnlyOutput(nodeStack.back().node, tmpl.substr(pos + tmpStopL, standaloneEnd - (pos + tmpStopL)),
              Node::FlagLambdaOnly, false, &nodeCount, limits, lineNo, charNo);
        }
        // Clear buffer
        buffer.clear();
        // Open new buffer
        inTag = false;
        skipUntil = standaloneEnd == std::string_view::npos ? pos + tmpStopL - 1 : standaloneEnd - 1;
        // Triple mustache
        if (!skip && inTripleTag && stop.compare("}}") == 0) {
          if (!matchesAt(tmpl, pos + 2, "}")) {
            std::ostringstream oss;
            oss << "Missing closing triple mustache delimiter at " << lineNo << ":" << charNo << " in tag starting at "
                << startLineNo << ":" << startCharNo;
    ...
    if (skipUntil == std::string_view::npos) {
      if (inTag && buffer.size() >= limits.maxTagBytes) {
        throw TokenizerException("Template tag size limit exceeded", lineNo, charNo);
      }
  ...
  if (inTag) {
    ...
    oss << "Unclosed tag at end of template, "
        << "starting at " << startLineNo << ":" << startCharNo;
    throw TokenizerException(oss.str(), startLineNo, startCharNo);
  } else if (nodeStack.size() > 1) {
    std::ostringstream oss;
    oss << "Unclosed section at end of template, depth was " << (nodeStack.size() - 1);
    ...
    appendNode(nodeStack.back().node, std::move(pendingNode), &nodeCount, limits, lineNo, charNo);
    buffer.clear();
  }
