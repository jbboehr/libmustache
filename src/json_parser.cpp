...
std::optional<std::string> normalizeFloatingSpelling(
    const std::string& spelling)
{
  ...
  while( position < normalized.size() &&
      normalized[position] >= '0' && normalized[position] <= '9' ) {
    ++position;
  }
  ...
  if( position < normalized.size() && normalized[position] != 'e' &&
      normalized[position] != 'E' ) {
  ...
  if( position < normalized.size() &&
      (normalized[position] == 'e' || normalized[position] == 'E') ) {
    ++position;
    if( position < normalized.size() &&
...
class JSONParseBudget {
  public:
    explicit JSONParseBudget(const Data::ParseLimits& limits) :
        limits_(limits), nodes_(0), stringBytes_(0), containerEntries_(0) {}
...
class MUSTACHE_LOCAL_CLASS Data::JSONDataBuilder final :
  ...
  public:
    ...
    bool number_float(number_float_t value,
        const string_t& spelling) override
    {
      beginValue();
      std::optional<std::string> normalized =
          normalizeFloatingSpelling(spelling);
    ...
    {
      if( frames_.empty() ||
          frames_.back().value.type() != Data::TypeMap ||
          frames_.back().key.has_value() ) {
        return false;
      }
  ...
  private:
    struct Frame {
      explicit Frame(Data value) : value(std::move(value)) {}
    ...
    {
      if( !frames_.empty() &&
          frames_.back().value.type() == Data::TypeArray ) {
        budget_.addContainerEntries(1);
      }
    ...
    void addValue(Data value)
    {
      ...
      if( parent.value.type() != Data::TypeMap ||
          !parent.key.has_value() ) {
        throw Exception("Invalid JSON data");
      }
    ...
    {
      if( frames_.empty() || frames_.back().value.type() != type ||
          frames_.back().key.has_value() ) {
        return false;
      }
...
Data Data::parseJSON(
    std::string_view string, const Data::ParseLimits& limits)
{
  JSONDataBuilder builder(limits);
  bool parsed = false;
  try {
    parsed = nlohmann::json::sax_parse(
        string.begin(), string.end(), &builder,
