...
void showUsage(std::ostream& output)
{
  output
      ...
      << "  -n, --repeat <count>      Render 1-" << maxRepetitions
      << " times (default: 1)\n"
...
bool readOptionValue(const std::vector<std::string>& arguments,
    std::size_t& index, std::string_view shortOption,
    std::string_view longOption, std::string& value)
{
  ...
  } else if( argument.size() > shortOption.size() &&
      argument.compare(0, shortOption.size(), shortOption) == 0 ) {
    value = argument.substr(shortOption.size());
    if( value.front() == '=' ) {
      throw std::runtime_error("Option " + std::string(shortOption) +
          " does not use '='; separate its value or use " +
  ...
  if( value.empty() ) {
    throw std::runtime_error(
        "Option " + std::string(shortOption) + " requires an argument");
  }
  return true;
}
...
std::size_t parseRepetitions(const std::string& value)
{
  ...
  for( const char character : value ) {
    if( character < '0' || character > '9' ) {
      throw std::runtime_error(
          "Render count must be a positive integer: " + value);
    }
    const std::size_t digit = static_cast<std::size_t>(character - '0');
    if( repetitions >
        (std::numeric_limits<std::size_t>::max() - digit) / 10 ) {
      throw std::runtime_error(
          "Render count must be between 1 and " +
          std::to_string(maxRepetitions) + ": " + value);
    }
    repetitions = repetitions * 10 + digit;
  }
  if( repetitions == 0 || repetitions > maxRepetitions ) {
    throw std::runtime_error("Render count must be between 1 and " +
        std::to_string(maxRepetitions) + ": " + value);
  }
  return repetitions;
}
...
void addPartial(Options& options, const std::string& value)
{
  ...
  if( separator == std::string::npos || separator == 0 ||
    ...
    throw std::runtime_error(
        "Partial must use the form <name>=<file>: " + value);
  }
...
Options parseOptions(const std::vector<std::string>& arguments)
{
  ...
  for( std::size_t index = 0; index < arguments.size(); ++index ) {
    ...
    std::string value;
    if( readOptionValue(
            arguments, index, "-t", "--template", value) ) {
      ...
      options.repetitionsSet = true;
                   ...
                   arguments, index, "-l", "--partial", value) ) {
...
std::string readFile(const std::string& path, std::string_view description,
    std::size_t maxBytes)
{
  errno = 0;
  ...
  if( !input ) {
    ...
    throw std::runtime_error("Cannot open " + std::string(description) +
        " '" + path + "': " + systemMessage(errorNumber));
  }
  ...
  for( ;; ) {
    ...
    if( count > 0 ) {
      ...
      if( contents.size() > maxBytes ||
          bytes > maxBytes - contents.size() ) {
        throw std::runtime_error(std::string(description) +
            " exceeds its input byte limit: " + path);
      }
    ...
    if( !input ) {
      throw std::runtime_error(
          "Cannot read " + std::string(description) + " '" + path + "'");
    }
  }
  return contents;
}
...
DataFormat detectDataFormat(const std::string& path)
{
  ...
  std::string extension = separator == std::string::npos ||
          (directorySeparator != std::string::npos &&
  ...
  throw std::runtime_error(
      "Unsupported data file type for '" + path +
      "'; expected .json, .yml, or .yaml");
}

mustache::Data readData(const Options& options,
    const mustache::Data::ParseLimits& limits)
{
  ...
  const std::string source =
      readFile(options.dataPath, "data file", limits.maxInputBytes);
  if( source.empty() ) {
...
mustache::PartialMap readPartials(const Options& options,
    const mustache::Tokenizer::Limits& limits)
{
  mustache::PartialMap partials;
  for( const auto& entry : options.partialPaths ) {
    const std::string source = readFile(
        entry.second, "partial '" + entry.first + "'", limits.maxInputBytes);
    partials.emplace(entry.first, mustache::compile(source, limits));
  }
  return partials;
}

void writeOutput(const std::string& path, const std::string& rendered,
    std::ostream& output)
{
  if( path.empty() ) {
...
int render(const Options& options, std::ostream& output)
{
  ...
  const std::string source = readFile(
      options.templatePath, "template file", tokenizerLimits.maxInputBytes);
  const mustache::CompiledTemplate compiled =
      mustache::compile(source, tokenizerLimits);
  const mustache::PartialMap partials =
      readPartials(options, tokenizerLimits);
  const mustache::Data data = readData(options, dataLimits);

  std::string rendered;
  for( std::size_t iteration = 0;
      iteration < options.repetitions; ++iteration ) {
...
void reportTokenizerFailure(std::ostream& error,
    const mustache::TokenizerException& exception) noexcept
{
  try {
    error << "mustachec: " << exception.what();
    if( exception.lineNo > 0 && exception.charNo > 0 ) {
      error << " at line " << exception.lineNo
            << ", character " << exception.charNo;
    }
    error << '\n';
  } catch( ... ) {
  }
