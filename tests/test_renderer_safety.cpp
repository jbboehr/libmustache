...
template <typename Callable> void expectException(const char * label, const char * expected, Callable&& callable)
{
  bool rejected = false;
  try {
    ...
    std::string invoke(std::string *, mustache::Renderer * renderer) override
    {
      *retained = renderer;
      ...
      try {
        ...
        mutationRejected = std::string(exception.what()) == "Renderer is already rendering";
      }
      if (!mutationRejected) {
        throw mustache::Exception("Active renderer mutation was not rejected");
      }
      ...
      try {
        ...
        reentryRejected = std::string(exception.what()) == "Renderer is already rendering";
      }
      if (!reentryRejected) {
        throw mustache::Exception("Active renderer re-entry was not rejected");
      }
...
class ScopedRenderingLambda : public mustache::Lambda {
  public:
    ScopedRenderingLambda(mustache::LambdaRenderContext * retained, std::string * observed) :
        retained(retained),
        observed(observed)
    {}
    ...
    {
      if (!context.active()) {
        throw mustache::Exception("Scoped lambda received an inactive context");
      }
      *retained = context;
      ...
      if (context.render(name) != buffered) {
        throw mustache::Exception("Scoped lambda render overloads disagreed");
      }
      return buffered;
    }
...
class ThrowingScopedLambda : public mustache::Lambda {
  public:
    explicit ThrowingScopedLambda(mustache::LambdaRenderContext * retained) :
        retained(retained)
    {}
...
class InnerScopedLambda : public mustache::Lambda {
  public:
    explicit InnerScopedLambda(mustache::LambdaRenderContext * retained) :
        retained(retained)
    {}
...
class OuterScopedLambda : public mustache::Lambda {
  public:
    OuterScopedLambda(mustache::LambdaRenderContext * retained, mustache::LambdaRenderContext * innerRetained) :
        retained(retained),
        innerRetained(innerRetained)
    {}
    ...
    std::string invoke(std::string_view, mustache::LambdaRenderContext context) override
    {
      *retained = context;
      ...
      inner.startSequence = "{{";
      inner.stopSequence = "}}";
      inner.children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypeStop, "inner"));
      if (!context.render(inner).empty()) {
        throw mustache::Exception("Nested scoped lambda produced unexpected output");
      }
      if (innerRetained->active()) {
        throw mustache::Exception("Nested lambda context remained active after its callback");
      }
      ...
      try {
        ...
        innerRejected = std::string(exception.what()) == "Lambda render context is no longer active";
      }
      if (!innerRejected || !context.active()) {
        throw mustache::Exception("Nested lambda contexts were not isolated by callback frame");
      }
...
void testLimitDefaultsAndOutputAccounting()
{
  ...
  expect(defaults.maxOutputBytes == 64 * 1024 * 1024, "default render output limit changed");
  expect(defaults.maxNestingDepth == 256, "default render nesting limit changed");
  expect(defaults.maxNodeVisits == 1000000, "default render node-visit limit changed");
  expect(defaults.maxLambdaTemplateBytes == 64 * 1024 * 1024, "default lambda-template limit changed");
  ...
  expect(mustache::render(escaped, lessThan, limits) == "&lt;", "exact escaped-output limit rejected valid output");

  limits.maxOutputBytes = 3;
  expectException("escaped output limit", "Render output byte limit exceeded", [&]() {
  ...
  limits.maxOutputBytes = 3;
  expectException("literal output limit", "Render output byte limit exceeded", [&]() {
    static_cast<void>(mustache::render(plain, mustache::Data::null(), limits));
  });

  mustache::Data callbackData =
      mustache::Data::object({{"value", mustache::Data::lambda(std::make_unique<CallbackRenderingLambda>())}});
  const mustache::CompiledTemplate callback = mustache::compile("{{#value}}{{/value}}");
  limits = mustache::RenderLimits();
  ...
  expectException("aggregate callback-output limit", "Render output byte limit exceeded", [&]() {
...
void testDepthAndWorkLimits()
{
  ...
  expect(mustache::render(plain, data, limits) == "x", "exact render nesting limit rejected a root and child");
  ...
  expect(mustache::render(plain, data, limits) == "x", "exact node-visit limit rejected a root and child");
  limits.maxNodeVisits = 1;
  expectException("render node-visit limit", "Render node visit limit exceeded", [&]() {
    static_cast<void>(mustache::render(plain, data, limits));
  });
  ...
  expectException("recursive partial nesting", "Render nesting limit exceeded", [&]() {
  ...
  layoutPartials.emplace("one", mustache::compile(nestedPartial("{{>two}}", 42)));
  layoutPartials.emplace("two", mustache::compile(nestedPartial("{{>three}}", 42)));
  layoutPartials.emplace("three", mustache::compile(nestedPartial("x", 42)));
  const mustache::Data layoutData = mustache::Data::object({{"ok", mustache::Data::boolean(true)}});
...
void testPartialIndentationOutputAccounting()
{
  ...
  limits.maxOutputBytes = 4;
  expectException("partial indentation output limit", "Render output byte limit exceeded", [&]() {
    static_cast<void>(mustache::render(source, data, partials, limits));
  });

  mustache::Data lambdaData =
      mustache::Data::object({{"value", mustache::Data::lambda(std::make_unique<FixedLambda>("a\nb"))}});
  ...
  invalidRoot.children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypePartial, "lines"));
  mustache::Mustache engine;
  std::string output;
  expectException("manual partial indentation bytes", "Invalid standalone partial indentation metadata", [&]() {
    engine.render(&invalidRoot, &data, NULL, &output);
  });

  mustache::Node lambdaRoot;
  lambdaRoot.type = mustache::Node::TypeRoot;
  std::unique_ptr<mustache::Node> section = std::make_unique<mustache::Node>(mustache::Node::TypeSection, "section");
  ...
  section->children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypePartial, "lines"));
  section->children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypeStop, "section"));
  lambdaRoot.children.push_back(std::move(section));
  mustache::Data sectionData =
      mustache::Data::object({{"section", mustache::Data::lambda(std::make_unique<FixedLambda>(""))}});
  output.clear();
  expectException("lambda source partial indentation bytes", "Invalid standalone partial indentation metadata", [&]() {
...
{
  mustache::Data data = mustache::Data::object({{"value", mustache::Data::lambda(std::make_unique<FixedLambda>("x"))}});
  const mustache::CompiledTemplate twice = mustache::compile("{{value}}{{value}}");

  mustache::RenderLimits limits;
  limits.maxLambdaTemplateBytes = 2;
  expect(mustache::render(twice, data, limits) == "xx", "exact aggregate lambda-template limit rejected valid output");
  limits.maxLambdaTemplateBytes = 1;
  expectException("aggregate lambda-template limit", "Render lambda template byte limit exceeded", [&]() {
    static_cast<void>(mustache::render(twice, data, limits));
  });

  mustache::Data sectionData =
      mustache::Data::object({{"value", mustache::Data::lambda(std::make_unique<FixedLambda>(""))}});
  const mustache::CompiledTemplate section = mustache::compile("{{#value}}abcd{{/value}}");
  limits = mustache::RenderLimits();
  ...
  expectException("section-lambda input limit", "Render lambda template byte limit exceeded", [&]() {
...
void testLambdaNodeAccounting()
{
  mustache::Data data = mustache::Data::object(
      {{"value", mustache::Data::lambda(std::make_unique<FixedLambda>("{{#missing}}{{!a}}{{!b}}{{/missing}}"))}});
  const mustache::CompiledTemplate source = mustache::compile("{{value}}");

  mustache::RenderLimits limits;
  limits.maxNodeVisits = 9;
  expect(mustache::render(source, data, limits).empty(), "exact parsed-lambda node budget rejected valid output");
  limits.maxNodeVisits = 8;
  expectException("parsed lambda node budget", "Render node visit limit exceeded", [&]() {
...
void testScopedLambdaContext()
{
  ...
  expectException("default lambda context", "Lambda render context is no longer active", [&]() {
    static_cast<void>(inactive.render(literal));
  });

  mustache::LambdaRenderContext retained;
  std::string observed;
  mustache::Data data = mustache::Data::object({{"name", mustache::Data::string("safe")},
      {"scoped", mustache::Data::lambda(std::make_unique<ScopedRenderingLambda>(&retained, &observed))}});
  const mustache::CompiledTemplate scoped = mustache::compile("{{#scoped}}original{{/scoped}}");
  expect(mustache::render(scoped, data) == "safe", "scoped lambda rendering changed callback output");
  expect(observed == "original", "scoped lambda did not receive exact section text");
  expect(!retained.active(), "retained lambda context remained active after callback completion");
  expectException("retained scoped lambda context", "Lambda render context is no longer active", [&]() {
    static_cast<void>(retained.render(literal));
  });

  mustache::LambdaRenderContext failed;
  mustache::Data failureData =
      mustache::Data::object({{"fail", mustache::Data::lambda(std::make_unique<ThrowingScopedLambda>(&failed))}});
  const mustache::CompiledTemplate failure = mustache::compile("{{#fail}}{{/fail}}");
  expectException("throwing scoped lambda", "Scoped lambda failed", [&]() {
    static_cast<void>(mustache::render(failure, failureData));
  });
  expect(!failed.active(), "throwing lambda left its retained context active");
  expectException("retained context after callback exception", "Lambda render context is no longer active", [&]() {
    static_cast<void>(failed.render(literal));
  });

  mustache::LambdaRenderContext outerRetained;
  mustache::LambdaRenderContext innerRetained;
  mustache::Data nestedData = mustache::Data::object({{"name", mustache::Data::string("outer")},
      {"inner", mustache::Data::lambda(std::make_unique<InnerScopedLambda>(&innerRetained))},
      {"outer", mustache::Data::lambda(std::make_unique<OuterScopedLambda>(&outerRetained, &innerRetained))}});
  const mustache::CompiledTemplate nested = mustache::compile("{{#outer}}{{/outer}}");
  expect(mustache::render(nested, nestedData) == "outer", "nested scoped lambda rendering failed");
...
void testFailureStateAndCallbackWindow()
{
  ...
  mustache::Data data = mustache::Data::object({{"name", mustache::Data::string("outer")},
      {"inner", mustache::Data::object({{"name", mustache::Data::string("inner")}})},
      {"recover", mustache::Data::lambda(std::make_unique<RecoveringLambda>(&retained))}});
  ...
  expect(output == "outer!", "a caught callback failure left output or data-stack state corrupted");
  expect(retained == &engine.renderer, "the section callback did not receive its renderer");

  mustache::Node literal(mustache::Node::TypeOutput, "late");
  std::string lateOutput;
  expectException("retained lambda renderer", "Lambda renderer is no longer active", [&]() {
  ...
  expectException("failed renderer operation", "Render output byte limit exceeded", [&]() {
    engine.renderer.render();
  });
  ...
  engine.renderer.init(&root, &data, NULL, &output);
  engine.renderer.render();
  expect(output == "outer!", "a renderer could not be reused after limit exhaustion");
  ...
  expectException("partial caller output on failure", "Render output byte limit exceeded", [&]() {
    engine.renderer.render();
  });
  expect(output == "AAAA", "a render failure did not retain the documented output prefix");
}

} // namespace

static_assert(
    !std::is_copy_constructible<mustache::Renderer>::value, "a renderer must not duplicate borrowed operation state");
static_assert(!std::is_copy_assignable<mustache::Renderer>::value, "a renderer must not copy borrowed operation state");
static_assert(!std::is_move_constructible<mustache::Renderer>::value, "an active renderer must not be movable");
static_assert(!std::is_move_assignable<mustache::Renderer>::value, "an active renderer must not be move assigned");
...
static_assert(
    !std::is_abstract<ScopedRenderingLambda>::value, "new lambdas must not need to implement the legacy renderer hook");
