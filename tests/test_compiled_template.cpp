...
void testHandleOwnershipAndReuse()
{
  ...
  expect(original.empty() && !original,
      "a default compiled template was not empty");
...
void testMemberConfigurationAndLimits()
{
  ...
  expect(engine.render(compiled, data) == "configured",
      "failed compilation damaged an existing compiled template");
}

void testCompiledPartials()
{
  mustache::CompiledTemplate root =
      mustache::compile("{{>outer}}|{{>missing}}");
...
static_assert(std::is_copy_constructible<mustache::CompiledTemplate>::value,
    "compiled templates must be copy constructible");
static_assert(std::is_copy_assignable<mustache::CompiledTemplate>::value,
    "compiled templates must be copy assignable");
static_assert(
    std::is_nothrow_move_constructible<mustache::CompiledTemplate>::value,
    "compiled templates must be nothrow move constructible");
static_assert(
    std::is_nothrow_move_assignable<mustache::CompiledTemplate>::value,
