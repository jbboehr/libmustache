...
class CountingLambda : public mustache::Lambda {
  public:
    explicit CountingLambda(int * destructions) :
...
void testDataCopyAndMoveConstruction()
{
  ...
  expect(source.type() == mustache::Data::TypeNone, "Data move construction did not reset the source type");
  expect(moved.type() == mustache::Data::TypeMap, "Data move construction did not transfer the type");
  expect(moved.objectItems().size() == 2, "Data move construction did not transfer map entries");
  const mustache::Data * name = moved.find("name");
  expect(name != NULL && name->stringValue() == "changed", "Data move construction did not preserve a string value");
  const mustache::Data * nested = moved.find("values");
  expect(
      nested != NULL && nested->arrayItems().size() == 1 && nested->arrayItems().front().stringValue() == "rock hard",
...
void testDataMoveAssignment()
{
  ...
  expect(source.type() == mustache::Data::TypeNone, "Data move assignment did not reset the source type");
  expect(destination.type() == mustache::Data::TypeList, "Data move assignment did not transfer the type");
  expect(destination.listItems().size() == 1 && destination.listItems().front().stringValue() == "new",
...
void testDataLambdaOwnership()
{
  int destructions = 0;
  {
    mustache::Data source = mustache::Data::lambda(std::make_unique<CountingLambda>(&destructions));
    mustache::Data shared(source);
    expect(source.lambdaValue() == shared.lambdaValue(), "Data copying did not explicitly share lambda ownership");
    expect(shared.lambdaValue()->invoke() == "lambda", "a copied Data lambda could not be invoked");

    mustache::Data destination = mustache::Data::lambda(std::make_unique<CountingLambda>(&destructions));

    destination = std::move(source);

    expect(destructions == 1, "Data move assignment did not destroy the destination lambda");
    expect(source.type() == mustache::Data::TypeNone && source.lambdaValue() == NULL,
        "Data lambda move assignment did not reset the source");
    expect(destination.type() == mustache::Data::TypeLambda && destination.lambdaValue() != NULL &&
            destination.lambdaValue()->invoke() == "lambda",
        "Data lambda move assignment did not transfer ownership");
  }
  expect(destructions == 2, "Data lambda sharing did not destroy each callback exactly once");
}

void testNodeMoveConstruction()
{
  mustache::Node source(mustache::Node::TypeSection, "person.name", mustache::Node::FlagEscape);
  source.children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypeVariable, "value"));
  source.child = std::make_unique<mustache::Node>(mustache::Node::TypeOutput, "container child");
  source.partials.emplace("card", std::make_unique<mustache::Node>(mustache::Node::TypeVariable, "title"));
  source.startSequence = "<%";
  source.stopSequence = "%>";

  mustache::Node moved(std::move(source));

  expect(source.type == mustache::Node::TypeNone, "Node move construction did not reset the source type");
  expect(!source.data.has_value() && source.dataParts.empty() && source.children.empty() && source.child == NULL &&
          source.partials.empty() && !source.startSequence.has_value() && !source.stopSequence.has_value(),
      "Node move construction left owned state in the source");
  expect(moved.type == mustache::Node::TypeSection && moved.flags == mustache::Node::FlagEscape,
      "Node move construction did not transfer scalar state");
  expect(moved.data.has_value() && *moved.data == "person.name" && moved.dataParts.size() == 2,
      "Node move construction did not transfer data state");
  expect(moved.children.size() == 1 && moved.child != NULL && moved.child->data.has_value() &&
  ...
  expect(partial != moved.partials.end() && partial->second != NULL && partial->second->data.has_value() &&
          *partial->second->data == "title",
      "Node move construction did not transfer internal partials");
  expect(moved.startSequence.has_value() && moved.stopSequence.has_value() && *moved.startSequence == "<%" &&
          *moved.stopSequence == "%>",
...
void testNodeMoveAssignment()
{
  ...
  source.children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypeComment, "child"));

  mustache::Node destination(mustache::Node::TypeVariable, "old.value");
  destination.children.push_back(std::make_unique<mustache::Node>(mustache::Node::TypeComment, "old child"));
  destination.partials.emplace("old", std::make_unique<mustache::Node>(mustache::Node::TypeVariable, "old partial"));
  destination = std::move(source);

  expect(source.type == mustache::Node::TypeNone && !source.data.has_value() && source.dataParts.empty() &&
          source.children.empty(),
      "Node move assignment did not reset the source");
  expect(destination.type == mustache::Node::TypeVariable && destination.data.has_value() &&
          *destination.data == "new.value",
      "Node move assignment did not transfer data");
  expect(destination.children.size() == 1 && *destination.children.front()->data == "child",
      "Node move assignment did not replace owned children");
  expect(destination.partials.empty(), "Node move assignment retained the destination's old partials");
}
...
void testOwnedNodeRendering()
{
  ...
  container.child = std::make_unique<mustache::Node>(mustache::Node::TypeOutput, "owned child");
  renderer.init(&container, &data, NULL, &output);
  renderer.render();
  expect(output == "owned child", "Renderer did not follow the owned container child");
  ...
  try {
    ...
    rejected = std::string(exception.what()).find("Empty tree node") != std::string::npos;
  }
