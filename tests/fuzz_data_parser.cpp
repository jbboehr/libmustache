...
void validateData(
    const mustache::Data& data, const mustache::Data::ParseLimits& limits, std::size_t depth, ParsedUsage& usage)
{
  ...
  switch (data.type()) {
    ...
    case mustache::Data::TypeString:
      addUsage(usage.stringBytes, data.stringValue().size(), limits.maxStringBytes);
      static_cast<void>(data.toString());
      return;
    case mustache::Data::TypeList:
      for (const mustache::Data& child : data.listItems()) {
        addUsage(usage.containerEntries, 1, limits.maxContainerEntries);
        validateData(child, limits, depth + 1, usage);
      }
      return;
    case mustache::Data::TypeMap:
      for (const mustache::Data::Map::value_type& member : data.objectItems()) {
        addUsage(usage.containerEntries, 1, limits.maxContainerEntries);
        addUsage(usage.stringBytes, member.first.size(), limits.maxStringBytes);
...
void validateParsedData(const mustache::Data& data, const mustache::Data::ParseLimits& limits)
{
  ParsedUsage usage;
  ...
  if (copiedUsage.nodes != usage.nodes || copiedUsage.stringBytes != usage.stringBytes ||
      copiedUsage.containerEntries != usage.containerEntries || copy.type() != data.type()) {
    std::abort();
  }
}
