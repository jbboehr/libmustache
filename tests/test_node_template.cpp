...
template <typename Callable> void expectException(Callable callable, const char * expected, const char * message)
{
  try {
    callable();
...
std::unique_ptr<mustache::Node> makeNode(mustache::Node::Type type, const std::string& data, int flags = 0)
{
...
void testCompatibilityOutput()
{
  ...
  root.children.push_back(makeNode(mustache::Node::TypeVariable, "value", mustache::Node::FlagEscape));

  std::unique_ptr<mustache::Node> section = makeNode(mustache::Node::TypeSection, "section");
  section->children.push_back(makeNode(mustache::Node::TypeOutput, std::string("a\0b", 3)));
  section->children.push_back(makeNode(mustache::Node::TypeStop, "section"));
  ...
  expect(
      root.to_template_string("{{", "}}") == fullExpected, "bounded node reconstruction changed compatibility output");
...
void testDefaultLimits()
{
  ...
  expect(limits.maxOutputBytes == 64 * 1024 * 1024, "default template reconstruction output limit changed");
  expect(limits.maxNestingDepth == 64, "default template reconstruction nesting limit changed");
  expect(limits.maxNodes == 100001, "default template reconstruction node limit changed");
}
...
void testOutputLimit()
{
  ...
  expectException(
      [&output, &limits]() {
        static_cast<void>(output.to_template_string("{{", "}}", limits));
      },
      "Template reconstruction size limit exceeded", "oversized template reconstruction output was accepted");
...
void testNestingLimit()
{
  ...
  root.children.front()->children.push_back(makeNode(mustache::Node::TypeOutput, "value"));

  mustache::Node::TemplateStringLimits limits;
  limits.maxNestingDepth = 3;
  expect(root.to_template_string("{{", "}}", limits) == "{{#outer}}value",
  ...
  expectException(
      ...
      "Template node nesting limit exceeded", "oversized template reconstruction nesting was accepted");
  ...
  expectException(
      ...
      "Template node nesting limit exceeded", "a zero template reconstruction nesting limit was unlimited");
  ...
  for (std::size_t depth = 0; depth < 256; ++depth) {
    parent->children.push_back(makeNode(mustache::Node::TypeSection, "section"));
  ...
  expectException(
      [&excessive, &limits]() {
        static_cast<void>(excessive.to_template_string("{{", "}}", limits));
      },
      "Template node nesting limit exceeded", "configured limits bypassed the reconstruction stack ceiling");
}
...
void testNodeLimit()
{
  ...
  expectException(
      [&root, &limits]() {
        static_cast<void>(root.children_to_template_string("{{", "}}", limits));
      },
      "Template node count limit exceeded", "a skipped closing node bypassed the reconstruction work budget");
  ...
  expectException(
      ...
      "Template node count limit exceeded", "a zero template reconstruction node limit was unlimited");
}
...
void testMalformedNodesAndReuse()
{
  ...
  expectException(
      ...
      "Invalid node without data", "template reconstruction accepted a node without required data");
  ...
  expectException(
      ...
      "Invalid null child node", "template reconstruction accepted a null child");

  root.children.pop_back();
  root.children.push_back(makeNode(mustache::Node::TypeOutput, "suffix"));
  expect(
      root.to_template_string("{{", "}}") == "prefixsuffix", "a failed reconstruction left the source tree unusable");
}
